...
If a necessary condition is violated, the system raises an error and stops the motion.
For example, if the first point of a user-provided joint trajectory differs too much
from the robot's actual start configuration
(:math:`q(t=0) \neq q_c(t=0)`), a ``start_pose_invalid`` error is triggered.

All constants referenced in the equations below are provided in the
`Limits for Franka Emika Robot (FER)`_ and `Limits for Franka Research 3 (FR3)`_ sections.
...
This parameter is used to determine the elbow flip direction.
...
.. important::
    ...
    .. math::
      ...
      \dot{q_i}(q_i)_{max} = \min(\dot{q}_{max,i}, \max(0, -\dot{q}_{offset,i}
      + \sqrt{\max(0, 2 \cdot \ddot{q}_{dec,i} (q_{max,i} - q_i))}))

    .. math::

      \dot{q_i}(q_i)_{min} = \max(\dot{q}_{min,i}, \min(0, \dot{q}_{offset,i}
